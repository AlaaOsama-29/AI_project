...
# Force UTF-8 output so Unicode box-drawing characters don't crash on Windows cp1252 consoles.
...
# ─── Voice Capture (from AI_interview) ───────────────────
_audio_capture = None
_stt_engine    = None
_use_voice     = False
...
def _init_voice(topic: str = "") -> bool:
    ...
    try:
        ai_dir = Path(__file__).resolve().parent / "AI_interview"
...
def speak(spk, text):
    ...
    try:
        ...
        if not safe:
            return
        tts_max = 420
        to_speak = safe[:tts_max].rsplit(" ", 1)[0] + " ... Rest is on screen." if len(safe) > tts_max else safe
...
def countdown_timer(seconds, stop_event):
    ...
    if timed_out:
        ...
        stop_event.set()  # Signal mic to stop recording
...
def get_answer(spk, question, q_num, level, time_limit,
    ...
    while True:
        ...
        try:
            if _use_voice:
                # Start countdown timer in background while mic records
                ...
                if answer is None:
                    # Voice failed — fall back to keyboard for this question
                    print(f"  {YELLOW}No voice detected — type your answer:{RESET}")
...
def run_interview(student_name: str = None, session_id: str = "",
    ...
    topics = {
        ...
        "7": "DevOps and Cloud", "8": "Machine Learning",
        "9": "Data Science and Statistics", "10": "Cybersecurity",
        "11": "Operating Systems", "12": "Computer Networks",
        "13": "Mobile Development (Android/iOS)",
        "14": "Software Testing and QA", "15": "System Design",
    ...
    if not topic:
        ...
        while True:
            choice = input(f"\n  {CYAN}>{RESET}  Enter number (1-{len(topics)}) or C for custom: ").strip()
            if choice.lower() == "c":
            ...
            if choice in topics:
                topic = topics[choice]
                break
            print(f"  {RED}Please enter a number from 1 to {len(topics)} or C for custom.{RESET}")

    # ── Init voice AFTER topic is chosen (domain prompt needs topic) ──
    _init_voice(topic)
    ...
    if _use_voice:
        _log("INFO", f"{GREEN}Voice mode ON{RESET} — speak your answers into the mic.")
    else:
        _log("INFO", f"{YELLOW}Keyboard mode{RESET} — type your answers.")
    time.sleep(1)
    ...
    for idx in range(total):
        ...
        print(f"  {YELLOW}[FALLBACK]{RESET}" if "fallback" in reason.lower() or "random" in reason.lower()
              else f"  {GREEN}[AI GENERATED]{RESET}")
        ...
        if answer:
            print(f"\n  {DIM}Answer recorded:{RESET} {WHITE}{answer}{RESET}\n")
        ...
        results.append({"q_num": q_num, "level": level, "elapsed": elapsed})
